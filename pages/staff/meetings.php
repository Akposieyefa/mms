                              ...
                              <div class="card">
                                 <div class="card-header">
                                    ...
                                    <div class="">
						                  ...
						                  <?php 
						$cnt = 1;
						if($results = $meeting->meetings("SELECT `meetings`.*, `meetingtype`.`name` FROM `meetings` LEFT JOIN `meetingtype` ON `meetings`.`meetingTypeId`=`meetingtype`.`id` WHERE `meetingTypeId`='1' OR `meetingTypeId`='2' OR `meetingTypeId`='3' OR `meetingTypeId`='4'"))
						{
							...
							?>  
                              ...
                              <div class="card">
                                 <div class="card-header">
                                    ...
                                    <?php 
                  $cnt = 1;
                  if($results = $meeting->fetchmeetingRequest("SELECT `meetingrequest`.*, `users`.`name`, `users`.`email` FROM `meetingrequest` LEFT JOIN `users` ON `meetingrequest`.`userId`=`users`.`id` WHERE `staffemail`=?", [$_SESSION['userEmail']]))
                  {
                  foreach($results as $result)
                  {       
                     ?>  
                                      <tr>
                                        ...
                                        <td><?php echo $result->name. ' ' .$result->email;?></td>
                                        <td><?php echo htmlentities($result->meetingDate).' '.$result->meetingTime;?></td>s
                                         <td><?php echo ($result->status) ? "<span class='text-info'>".$result->status."</span>" : "--" ;?></td>
                                         <td><?php echo ($result->status == 'pending') ? '<a href="meetings.php?acceptid='.$result->id.'&email='.$result->email.'&purpose='.$result->purpose.'" class="text-success" onclick="return confirm("Accepting this meeting request")">Accept</a> | <a href="meetings.php?declineid='.$result->id.'&email='.$result->email.'&purpose='.$result->purpose.'" class="text-danger" onclick="return confirm("Declining this meeting request")">Decline</a>' : '' ;?></td>
                                      </tr>
                                      <?php $cnt=$cnt+1; }} ?>
